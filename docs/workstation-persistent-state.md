...
Collie's env file holds the VAPID private key at mode `0600`. Verify it survived before deleting anything:
...
Then remove the originals so the assert sees empty (or absent) mount points:

```bash
rm -rf ~/.config/herdr ~/.local/state/collie
```
