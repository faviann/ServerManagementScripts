...
The playbook resolves the control node public key through the shared SSH key resolver, using the project-local `.ansible/ssh/proxmox_lxc.pub` key generated by `bootstrap.yml` unless an explicit override is provided.
